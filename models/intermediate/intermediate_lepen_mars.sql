WITH details_des_temps AS (
  SELECT 
    SAFE.PARSE_TIME('%H:%M:%S', CAST(Tranche__6h_9h__dur___e_ AS STRING)) AS Tranche_6h_9h,
    SAFE.PARSE_TIME('%H:%M:%S', CAST(Tranche__9h_18h__dur___e_ AS STRING)) AS Tranche_9h_18h,
    SAFE.PARSE_TIME('%H:%M:%S', CAST(Tranche__18h_24h__dur___e_ AS STRING)) AS Tranche_18h_24h,
    SAFE.PARSE_TIME('%H:%M:%S', CAST(Tranche__0h_6h__dur___e_ AS STRING)) AS Tranche_0h_6h
  FROM `omega-cider-448409-a9.projet_lewagon.antenne_lepen_mars`
  WHERE LOWER(TRIM(`D___tail_des_temps`)) = "total temps d\'antenne"
) 
