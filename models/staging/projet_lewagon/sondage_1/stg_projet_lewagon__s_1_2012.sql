...
renamed as (
 ...
 select
    ...
    SAFE_CAST(REPLACE(
        ...
        END, ',', '.') AS FLOAT64) AS jean_luc_melenchon_fg_float,
    ...
    SAFE_CAST(REPLACE(
        ...
        END, ',', '.') AS FLOAT64) AS francois_hollande_ps_float,

    SAFE_CAST(REPLACE(
        CASE 
            WHEN LENGTH(eva_joly_eelv) > 2 THEN SUBSTR(eva_joly_eelv, 1, LENGTH(eva_joly_eelv) - 2)
            ELSE eva_joly_eelv
        END, ',', '.') AS FLOAT64) AS eva_joly_eelv_float,
    ...
    SAFE_CAST(REPLACE(
        ...
        END, ',', '.') AS FLOAT64) AS francois_bayrou_modem_float,
    ...
    SAFE_CAST(REPLACE(
        ...
        END, ',', '.') AS FLOAT64) AS nicolas_dupont_aignan_dlr_float,
